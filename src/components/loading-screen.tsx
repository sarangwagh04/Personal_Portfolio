...
export function LoadingScreen({ onComplete }: { onComplete?: () => void }) {
    ...
    const animation = animate(0, 100, {
      ...
      onComplete: () => {
        ...
        if (onCompleteRef.current) {
          ...
          }, 150); // Small delay after hitting 100 before closing screen
        }
      }
    });
  ...
  return (
    <div className="w-full h-[100dvh] bg-[#070708] flex flex-col items-center justify-center overflow-hidden relative select-none">
      ...
      >
        ...
        <div className="relative w-full flex flex-col items-center justify-center mt-12 md:mt-0">
          ...
          {/* Background Loading Number */}
          <motion.div 
            initial={{ opacity: 0.1 }}
            animate={{ opacity: isDone ? 0 : 0.1 }}
            transition={{ duration: 0.2 }}
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[42vw] md:text-[300px] lg:text-[360px] font-['Anton'] text-white z-0 pointer-events-none flex items-center justify-center tracking-normal"
          >
